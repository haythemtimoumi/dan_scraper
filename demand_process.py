#!/usr/bin/env python
"""
Demand process for targeted records:
1. Fresh StockScores data
2. Fresh Price data  
3. Reused Rule1 data from current month
Only processes records where target = TRUE
"""
...
def run_demand_process():
    """Run demand process for targeted records"""
    current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    print(f"Starting demand process at {current_time}...\n")
    
    conn = psycopg2.connect(**DB_CONFIG)
    cursor = conn.cursor()
    
    try:
        # Get targeted tickers
        ...
        for i, (ticker_id, symbol, guru_id, list_type, last_action, per_portfolio) in enumerate(targeted_tickers, 1):
            ...
            try:
                ...
                # Reset target after processing
                cursor.execute("UPDATE scraper_tasks SET target = FALSE WHERE id = %s", (ticker_id,))
        ...
        print(f"\nDemand process completed: {success_count}/{len(targeted_tickers)} complete records created")
