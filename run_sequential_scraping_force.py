#!/usr/bin/env python
"""
Force sequential scraper for targeted records:
1. Only processes records where target = TRUE
2. Sets scrape_status = 'pending' for targeted records
3. Scrapes Rule1 + StockScores + Price data
"""
...
def run_sequential_scraping_force():
    """Run complete scraping for targeted records only"""
    current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    print(f"Starting force sequential scraping at {current_time}...\n")
    
    conn = psycopg2.connect(**DB_CONFIG)
    cursor = conn.cursor()
    
    try:
        # Get targeted tickers and set them to pending
        cursor.execute("SELECT id, symbol, guru_id, list_type, last_action, per_portfolio FROM scraper_tasks WHERE target = true")
        targeted_tickers = cursor.fetchall()
        
        if not targeted_tickers:
            print("No targeted tickers found")
            return 0
        
        # Set targeted tickers to pending
        cursor.execute("UPDATE scraper_tasks SET scrape_status = 'pending' WHERE target = true")
        conn.commit()
        
        print(f"Processing {len(targeted_tickers)} targeted tickers...\n")
        ...
        for i, (ticker_id, symbol, guru_id, list_type, last_action, per_portfolio) in enumerate(targeted_tickers, 1):
            print(f"[{i}/{len(targeted_tickers)}] Processing {symbol}...")
            ...
            try:
                ...
                # Update final timestamp and reset target
                cursor.execute("UPDATE scraper_tasks SET last_updated_at = %s, target = FALSE WHERE id = %s", (current_time, ticker_id))
        ...
        print(f"\nForce sequential scraping completed: {success_count}/{len(targeted_tickers)} complete records created")
