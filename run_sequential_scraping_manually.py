#!/usr/bin/env python
"""
Manual sequential scraper that creates complete records with:
1. Fresh Rule1 data
...
def run_sequential_scraping_manually():
    """Run complete scraping creating one record per ticker with all data (target tickers only)"""
    current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    print(f"Starting manual sequential scraping process at {current_time}...\n")
    
    conn = psycopg2.connect(**DB_CONFIG)
    cursor = conn.cursor()
    
    try:
        # Get target tickers with guru-specific data from guru_ticker_map
        cursor.execute("""
...
if __name__ == "__main__":
    run_sequential_scraping_manually()