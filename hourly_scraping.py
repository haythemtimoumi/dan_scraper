#!/usr/bin/env python
"""
Hourly scraper that creates complete records with:
1. Fresh Rule1 data
2. Fresh StockScores data
3. Fresh Price data
All combined into single records per ticker (HOURLY TICKERS ONLY)
"""
...
def run_hourly_scraping():
    """Run complete scraping creating one record per ticker with all data (hourly tickers only)"""
    current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    print(f"Starting hourly scraping process at {current_time}...\n")
    
    conn = psycopg2.connect(**DB_CONFIG)
    cursor = conn.cursor()
    
    try:
        # Get hourly tickers with guru-specific data from guru_ticker_map
        cursor.execute("""
            ...
            WHERE st.active = true AND st.scrape_type = 'hourly' AND st.scrape_status = 'pending'
            ORDER BY st.id, gtm.id
        """)
        hourly_tickers = cursor.fetchall()
        
        if not hourly_tickers:
            print("No hourly tickers found")
            return 0
        
        print(f"Processing {len(hourly_tickers)} hourly tickers...\n")
        ...
        for i, (ticker_id, symbol, guru_id, list_type, last_action, per_portfolio) in enumerate(hourly_tickers, 1):
            print(f"[{i}/{len(hourly_tickers)}] Processing {symbol}...")
