#!/usr/bin/env python
"""
Fresh data scraper for inactive tickers only
Scrapes Rule1, StockScores, and Price data for tickers where active = false
"""
...
def run_inactive_fresh_scraper():
    """Run fresh scraping for inactive tickers only"""
    current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    print(f"Starting fresh scraper for inactive tickers at {current_time}...\n")
    
    conn = psycopg2.connect(**DB_CONFIG)
    cursor = conn.cursor()
    
    try:
        # Get inactive tickers only
        cursor.execute("""
            SELECT st.id, st.symbol, st.guru_id, st.list_type, NULL as last_act, NULL as per_port 
            FROM scraper_tasks st 
            WHERE st.active = false
        """)
        inactive_tickers = cursor.fetchall()
        
        if not inactive_tickers:
            print("No inactive tickers found")
            return 0
        
        print(f"Processing {len(inactive_tickers)} inactive tickers...\n")
        ...
        for i, (ticker_id, symbol, guru_id, list_type, last_action, per_portfolio) in enumerate(inactive_tickers, 1):
            print(f"[{i}/{len(inactive_tickers)}] Processing {symbol}...")
        ...
        print(f"\nFresh scraping completed: {success_count}/{len(inactive_tickers)} records")
        
        # Send notification
        from firebase_notifier import FirebaseNotifier
        FirebaseNotifier.send_notification(
            title="Inactive Fresh Scraper Complete",
            body=f"Inactive fresh scraper: {success_count}/{len(inactive_tickers)} records",
            data={"script": "run_inactive_fresh_scraper", "success_count": str(success_count)}
        )
        ...
        from firebase_notifier import FirebaseNotifier
        FirebaseNotifier.send_notification(
            title="Inactive Fresh Scraper Failed",
            body=f"Inactive fresh scraper failed: {str(e)}",
            data={"script": "run_inactive_fresh_scraper", "error": str(e)}
        )
        raise
    finally:
        cursor.close()
