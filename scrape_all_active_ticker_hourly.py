#!/usr/bin/env python
"""
Process all active hourly tickers:
1. Fresh StockScores data
2. Fresh Price data  
3. Reused Rule1 data from current month
Only processes records where active = TRUE AND scrape_type = 'hourly'
"""
...
def run_active_hourly_process():
    """Run process for all active hourly tickers"""
    current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    print(f"Starting active hourly ticker process at {current_time}...\n")
    
    conn = psycopg2.connect(**DB_CONFIG)
    cursor = conn.cursor()
    
    try:
        # Get active hourly tickers
        ...
        active_hourly_tickers = cursor.fetchall()
        
        if not active_hourly_tickers:
            print("❌ No active hourly tickers found")
            return 0
        
        print(f"Processing {len(active_hourly_tickers)} active hourly tickers...\n")
        ...
        for i, (ticker_id, symbol, guru_id, list_type, last_action, per_portfolio) in enumerate(active_hourly_tickers, 1):
            print(f"[{i}/{len(active_hourly_tickers)}] Processing {symbol}...")
